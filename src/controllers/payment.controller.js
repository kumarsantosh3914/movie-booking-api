...
const create = async (req, res) => {
    try {
        ...
        const theatre = Theatre.findById(response.theatreId);
        ...
        sendMail(
            ...
            response.userId,
            `Your booking for ${movie.name} in  ${theatre.name} for ${response.noOfSeats} seats on ${response.timing} is successfull. You booking id is ${response.id}`,
        );
...
const getPaymentDetailsById = async (req, res) => {
    try {
        ...
        successResponseBody.data = response;
        successResponseBody.message = "Successfull fetched the booking and payment details";
        return res.status(STATUS.OK).json(successResponseBody);
    } catch (error) {
        if (error.err) {
            errorResponseBody.err = error.err;
